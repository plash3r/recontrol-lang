    ...
    pub fn tokenize(mut self) -> Result<Vec<Token>, Vec<LexError>> {
        ...
        while !self.is_at_end() {
            ...
            match self.advance() {
                ...
                '/' if self.match_char('=') => tokens.push(self.token(TokenKind::SlashEqual, "/=", line, column, 2)),
                '%' if self.match_char('=') => tokens.push(self.token(TokenKind::PercentEqual, "%=", line, column, 2)),

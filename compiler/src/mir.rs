...
impl MirLowerer {
    ...
    fn finalize_temporaries(function: &mut MirFunction, temp_start: usize) {
        ...
        // A temporary consumed by a terminator stays live across the outgoing
        // edge. Inserting StorageDead at a shared successor is unsound because
        // another predecessor may reach that block without ever making the temp
        // live. Compiler-generated terminator temps are therefore kept alive
        // conservatively; they are not user-visible and need no edge-local drop.
